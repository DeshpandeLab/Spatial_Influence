...
# calculate spatwt
coord<-data.frame(fsApply(output$fcs1, exprs)[,c("X_position","Y_position")],
               celltype= output$cell_clustering1m, # calculate influence with broader celltypes 
               sample_id=output$sample_ids)

# flip y-axis for ggplot to match cell mask and MCD images 
coord$Y_position <- 1000 - coord$Y_position 
coord$Site <- factor(output$Site[match(coord$sample_id,output$meta_data$sample_id)], levels=sitelevels)

coord$sample_id<-factor(coord$sample_id, levels=unique(coord$sample_id))

dim(coord) # 267239 cells x 5 (x,y coord, celltype, sample_id, site)


## Spatial weight calculations for each core =====
sampleID <- unique(coord$sample_id)

# CI quantification for broader celltypes
spatwt_1m <- do_CI_quantification(expr = coord,  # should contain sample_id, X, Y position, celltype in the df
                                      sampleID = sampleID,
                                      kernels ="gaussian",
                                      clusterlevels = unique(output$cell_clustering1m),
                                      sigma = 10) 
...
for (i in sampleID){   # by the core
  expr_k<- coord[coord$sample_id==i, ]
  dd <- expr_k[, c("celltype", "X_position", "Y_position")]
  rownames(dd)<- paste0(rownames(expr_k), ":", expr_k$celltype)
...
coord_filtered_2<- coord[id_2, ]
max_range<- max(coord$Y_position, coord$X_position)
w2 <- ppp(coord_filtered_2[coord_filtered_2$sample_id==select_core, ]$X_position,
          coord_filtered_2[coord_filtered_2$sample_id==select_core, ]$Y_position, 
...
pdf('./output/Figure2B.pdf', width=10, height=5)
par(mfrow = c(1, 2))
plot(densityplot)
...
plot(
  ...
  spatwt_Tcell_on_tum[spatwt_Tcell_on_tum$sample_id==select_core, ]$Y_position,
  col = colors,         
  pch = 20, cex = 0.6,             
  xlab = "",ylab = "",
  xaxt = "n",yaxt = "n",  
...
pdf('./output/Figure2C.pdf', height=2.7,width=7)
p2C<- ggplot(df_sub, aes(x= knn, y=gaussian, color=neighbor) ) + 
  geom_point() + 
  labs(x = "knn weights", y = "gaussian weights") +
  theme_minimal()+ 
  theme(
...
pdf('./output/Figure2D.pdf', height=3, width=2.5)
for (i in unique(data$neighbor)){
  p2D<- ggplot(data[data$neighbor%in%i,], aes(x = nn, y = weights, color = nn)) +
    geom_violin()  +
    geom_boxplot(width=0.2)+ 
    labs(x = "Nearest Neighbors", y = paste0(i, " cell influence")) +
    theme_minimal()+
...
data_for_plot_list <- lapply(1:nrow(pickcell), function(i) {
  ...
  reference <- coord[pickcell$refID[i],]
...
coord_2m<-data.frame(fsApply(output$fcs1, exprs)[,c("X_position","Y_position")],
                  celltype= output$cell_clustering2m, # calculate influence with subtypes
                  sample_id=output$sample_ids)

coord_2m$Site <- factor(output$Site[match(coord_2m$sample_id,output$sample_ids)], levels=sitelevels)

coord_2m$sample_id<-factor(coord_2m$sample_id, levels=unique(coord_2m$sample_id))

dim(coord_2m)

spatwt <- do_CI_quantification(expr = coord_2m,  # should contain sample_id, X, Y position, celltype in the df
                               sampleID = sampleID,
                               kernels ="gaussian",
...
# Define the index
idx <- list(
  which(output$cell_clustering2m == "CD8T" & output$sample_ids != 43),
  which(output$cell_clustering2m == "Tumor" & output$sample_ids != 43)
)
...
for (cell_type in cell_types) {
  idx <- which(output$cell_clustering2m == cell_type & 
                 output$sample_ids != 43)
  
  # Subset data
  spatwt_df <- spatwt[idx, ]
  spatwt_df$Site <- factor(output$meta_data$Site[match(spatwt_df$sample_id, output$meta_data$sample_id)], levels = sitelevels)
  spatwt_df$Patient <- factor(output$Patient[match(spatwt_df$sample_id, output$meta_data$sample_id)], levels = unique(output$Patient))
