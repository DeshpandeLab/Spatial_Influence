...
# Figure 2A - cell phenotype heatmap
...
counts_frac$Patient <- factor(df_output$Patient[match(counts_frac$sample_id,df_output$sample_id)], levels=unique(df_output$Patient))
...
pdf("./output/Figure2B.pdf", width=5, height =5)
print(plot_grid(p1, p0, rel_widths=c(7,1)))
dev.off()

pdf("./output/FigureS1F.pdf", width=9, height = 3.5)
print(p2)
print(p3)
print(p4)
dev.off()
...
pdf('./output/Figure2C.pdf',width=3.8, height=4)
